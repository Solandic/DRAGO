...
import { getAccessTokenFunction } from './utils/authentication';
...
export async function loadBanner() {
  ...
  const getAccessToken = getAccessTokenFunction();

  const store = await initStore({ preview: false, getAccessToken });
  ...
  // @ts-ignore
  const expiryPassed = (expiryDate || 0) > (createdPolicyTimestamp?.getTime() || 0);
