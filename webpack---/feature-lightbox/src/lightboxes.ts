...
const lightboxesFactory = (
	...
	const openLightbox: ILightbox['open'] = async (lightboxId, closeHandler, lightboxViewerContext) => {
	...
	return {
		...
		open(lightboxId, closeHandler = null, lightboxViewerContext) {
			...
			const openLightboxPromise = pendingLightboxPromise.then(() =>
				openLightbox(lightboxId, closeHandler, lightboxViewerContext)
			)
