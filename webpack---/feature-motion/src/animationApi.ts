...
import { getWebAnimation, getScrubScene, getElementAnimation } from '@wix/motion'
				...
				if ((namedEffect || customEffect) && (isPointer || isScroll)) {
					...
					if (triggerElement) {
						...
						triggerElements.forEach((sourceElement: Element) => {
							...
							const target = getDisplayedId(getFullId(targetId), getTemplateItemId(sourceElement.id))
							isScroll
								? addScroll(
			...
			scrollRootsMap.forEach((scenes, root) => {
				if (scenes.length) {
					if (hasViewTimeline) {
						...
						Promise.all(
							scenes.map(
								(scene: ScrubScrollScene & { ready: Promise<void> }) => scene.ready || Promise.resolve()
							)
						).then(() => {
							scrollManager.start()
						})
		...
		_createScrub(animation: ScrubEffectData, trigger: ScrubTriggerVariant & { element?: HTMLElement }) {
			return {
				...
				factory: (targetId: string, disabled = false) => {
					...
					if (animation.registerCustomEffect) {
						animation.registerCustomEffect(Array.isArray(scene) ? scene[0].getProgress : scene.getProgress)
					}
					return scene
				},
			}
