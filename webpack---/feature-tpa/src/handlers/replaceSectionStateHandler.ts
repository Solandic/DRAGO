	...
	(
		...
		getTpaHandlers() {
			const getLinkUtils = (routingInfo = routingLinkUtilsAPI.getLinkUtilsRoutingInfo()) => {
				const { metaSiteId, userFileDomainUrl, routersConfig, isMobileView, isPremiumDomain } = tpaSiteConfig
				...
				async appStateChanged(_compId, { state }: AppStateChangedMessage): Promise<void> {
					...
					if (parsedState) {
						const tpaGalleryStateCommandToAction: Record<string, () => void> = {
							...
							componentReady: () => {
								const readyReportsCount = propsStore.get(_compId).readyReportsCount ?? 0
								propsStore.update({
									[_compId]: {
										componentReady: true,
										// In some cases when 'componentReady' was re-reported we need to re-trigger initialization steps, for example #TB-9591
										readyReportsCount: readyReportsCount + 1,
									},
								})
							},
