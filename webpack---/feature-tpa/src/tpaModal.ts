...
import { isSSR, runtimeTpaCompIdBuilder, hideSiteRoot } from '@wix/thunderbolt-commons'
...
export const TpaModal = withDependencies(
	...
	(
	...
	): ITpaModal & IPageWillUnmountHandler & DynamicLoadFeature => {
		...
		return {
			init(_pageId: string) {
				mountUnmount.registerToPageDidUnmount(getCloseModalImpl())
			},
			pageWillUnmount() {
				getCloseModalImpl()()
			},
			...
			openModal(url: string, { width, height, title, theme }: OpenModalOptions, compId?: string): Promise<void> {
				...
				getCloseModalImpl()()
				cyclicTabbing.enableCyclicTabbing(modalCompId)

				const callerCompProps = compId ? props.get(compId) : null
				const appDefinitionId = callerCompProps ? callerCompProps.appDefinitionId : null
				const isAppWixTPA = wixTPAs[appDefinitionId]
				...
				return new Promise(async (resolve) => {
					...
					setCloseModalImpl((msg) => {
						...
						cyclicTabbing.disableCyclicTabbing(modalCompId)
