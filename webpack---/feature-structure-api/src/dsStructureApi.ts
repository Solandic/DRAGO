...
export const DsStructureAPI = withDependencies(
	[Structure, BaseStructureAPISym, named(EditorFeatureConfigSymbol, name), ExperimentsSymbol, Props],
	(
		...
		return {
			...
			addPageAndRootToRenderedTree: (pageId: string, contextId: string) => {
				...
				propsStore.update({
					[wrapperId]: { renderCompCssInSiteRoot },
					'site-root': { renderCompCssInSiteRoot },
				})
