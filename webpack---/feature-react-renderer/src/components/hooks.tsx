...
export const useControllerHook = (
	...
	const stateValues = getProps(stateRefs, isRepeatedComp, compId, displayedId, takePropsNotOnlyFromDisplayedAndFull)
...
export const getProps = (
	...
	const props = { ...(store.get(compId) ?? {}) }
	if (isRepeatedComp) {
		if (takePropsNotOnlyFromDisplayedAndFull) {
			const itemIds = getRepeaterItemsAncestorChain(getDisplayedItemId(displayedId)).reverse()
			itemIds.forEach((itemId) => {
				const currentProps = store.get(getDisplayedId(compId, itemId))
				Object.assign(props, currentProps)
			})
		} else {
			Object.assign(props, store.get(displayedId))
		}
	}
	return props
}
...
export const useProps = (
	...
	const isRepeatedComp = displayedId !== compId
	return getProps(propsStore, isRepeatedComp, compId, displayedId, takePropsNotOnlyFromDisplayedAndFull)
}
