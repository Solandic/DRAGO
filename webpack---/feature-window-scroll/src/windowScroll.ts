...
export const getHeaderElement = (headerComponentId: string | undefined, window: Window, experiments: Experiments) => {
	if (experiments['specs.thunderbolt.removeHeaderFooterWrappers']) {
		const [firstHeader] = Array.from(window.document.getElementsByTagName('header'))
		return firstHeader
	} else {
...
export const WindowScroll = withDependencies(
	...
	(
		...
		const getHeaderOffset = (distanceBetweenCompTopToBodyTop: number, wixAdsHeight: number) => {
			const headerElement = getHeaderElement(headerComponentId, window, experiments)
			if (!headerElement) {
				return 0
			}
