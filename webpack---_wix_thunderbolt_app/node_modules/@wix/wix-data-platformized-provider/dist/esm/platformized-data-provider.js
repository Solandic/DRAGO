                ...
                return this.traceWith('wix-data-api-call', { url })(() => {
                    ...
                    if (preferGet) {
                        ...
                        if (getUrl.length <= 2048) {
                            return this.httpClientProvider(options).get(getUrl, {
                                headers,
                                includeWixHeaders: true,
                            });
                        }
                    }
                    return this.httpClientProvider(options).post(url, data, {
                        headers,
                        includeWixHeaders: true,
                    });
                }).catch((error) => {
                    return this.traceWith('wix-data retry handler', {
                        retriesLeft,
                        httpStatus: error?.response?.status,
                        error,
                        isGetRequest: preferGet,
                    })(() => {
                        if (preferGet &&
                            retriesLeft > 0 &&
