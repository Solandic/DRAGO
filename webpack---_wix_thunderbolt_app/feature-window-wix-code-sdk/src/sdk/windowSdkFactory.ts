...
export function WindowSdkFactory({
	...
	function _openLightbox(
		...
		return new Promise((resolve, reject) => {
			...
			const closeHandler = (lightboxContext?: LightboxContext) => {
				resolve(lightboxContext ?? lightboxHandles[lightboxPageId].lightboxContext)
			}
	...
	function _getViewerContext() {
		if (!isPopup) {
			logSdkError('The current page is not a lightbox and therefore cannot get its context')
			return null
		}

		return getLightboxViewerContext(pageId)
	}
	...
	return {
		[namespace]: {
			...
			lightbox: {
				...
				getViewerContext: _getViewerContext,
