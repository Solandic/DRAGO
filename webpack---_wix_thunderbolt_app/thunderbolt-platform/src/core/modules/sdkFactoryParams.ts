...
import { createStyleUtils, getFullId, getTemplateItemId } from '@wix/thunderbolt-commons'
...
import { COMPONENTS_WITH_PORTAL_PREFIXES } from '@wix/thunderbolt-becky-types'
...
const SdkFactoryParams = (
	...
	return {
		getSdkFactoryParams: ({ compId, connection, compType, controllerCompId, role, getInstance, create$w, $wScope }) => {
			...
			const portalPrefix = COMPONENTS_WITH_PORTAL_PREFIXES[compType] || 'portal-'
			const portalId = `${portalPrefix}${connection?.compId || compId}`
			...
			function getSdkInstance() {
				return sdkInstancesCache.getSdkInstance({
					...
					controllerCompId,
					role,
					itemId: getTemplateItemId(compId),
				})
			}
			...
			const remove = () => {
				...
				if (modelsApi.getCompType(compId) === 'AppWidget') {
					const refComponentId = modelsApi.getParentId(compId)!
					handlers.stores.updateStructure({ [refComponentId]: { deleted: true } })
				}
			...
			const restore = () => {
				...
				if (modelsApi.getCompType(compId) === 'AppWidget') {
					const refComponentId = modelsApi.getParentId(compId)!
					handlers.stores.updateStructure({ [refComponentId]: { deleted: false } })
				}
			...
			const compHasResponsiveLayout = platformEnvData.site.experiments['specs.thunderbolt.collapseInWidgetStyle']
				? modelsApi.hasResponsiveLayout(getFullId(compId))
				: modelsApi.hasResponsiveLayout(compId)

			const isResponsive = platformEnvData.site.isResponsive || compHasResponsiveLayout
